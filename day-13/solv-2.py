...
def smudge(lines: List[str], sx: int, sy: int) -> List[str]:
    return [
        (line[:sx] + OPPOSITE[line[sx]] + line[sx + 1 :]) if sy == y else line
...
def get_horizontal_value(m: List[str]) -> int:
    for value in range(1, len(m)):
        ...
        for sy in range(len(top)):
            for sx in range(len(top[0])):
                if smudge(top, sx, sy) == list(reversed(bottom)):
                    return value
