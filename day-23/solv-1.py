...
SLOPE_LEFT = "<"
SLOPE_RIGHT = ">"
SLOPES: List[str] = [SLOPE_DOWN, SLOPE_UP, SLOPE_LEFT, SLOPE_RIGHT]
DIRECTIONS: List[C] = [(0, 1), (0, -1), (-1, 0), (1, 0)]
SLOPE_BY_DIRECTION: Dict[C, str] = dict(zip(DIRECTIONS, SLOPES))


@cache
def get_next_positions(grid: Tuple[str], position: C) -> Set[C]:
    positions = set()
    for direction, slope in SLOPE_BY_DIRECTION.items():
        ...
        if next_tile == FOREST:
            continue
        if next_tile in SLOPES and next_tile != slope:
            continue
